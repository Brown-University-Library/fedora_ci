...
if [ ! -f "$FEDORA_HOME/client/fcrepo-client-admin-3.8.2-SNAPSHOT.jar" ]; then
...
execWithTheseArgs() {
    exec_cmd="exec \"$java\" -Xms64m -Xmx96m \
            -cp \"$FEDORA_HOME\"/client/bin:\"$FEDORA_HOME\"/client/fcrepo-client-admin-3.8.2-SNAPSHOT.jar:\"$FEDORA_HOME\"/client/cxf-bundle-2.7.7.jar \
